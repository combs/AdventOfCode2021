...
while len(order):
    ...
    for board in boards:
        if checkBoard(board, drawn):
            print("BINGO! losing board:")
            print(board)
            print("drawn numbers:", drawn)
            summed = sum(undrawn(board, drawn))
            # print ("sum of undrawn numbers:", summed, "final answer:", drawn[-1] * summed)

            print(len(boards), "boards left")
            
            # there's a weird race condition here possible, I think, where two boards win simultaneously. Maybe.

            if len(boards)==1:
                print("last board standing:")
                print(boards[0])
                print("drawn numbers:", drawn)
                summed = sum(undrawn(boards[0], drawn))
                print ("sum of undrawn numbers:", summed, "final answer:", drawn[-1] * summed)
                sys.exit(0)
